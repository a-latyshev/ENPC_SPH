...
%Cfontact : remi.carmigniani@enpc.fr
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% plotParticlesPropertyWithBound(fluid,PROP,nameBar,propMin,propMax)
% Boosted with patch
function plotParticlesPropertyWithBound(part,PROP,nameBar,propMin,propMax)
global dr lspace;
global INFO;
global POS;
global FLUID BOUND;
...
colormap(c);
if size(version,2) ==size('7.5.0.342 (R2007b)',2)
    cbar = colorbar('YTickLabel',{[0.:0.1:1]*(propMax-propMin)+propMin});
else
    cbar = colorbar;
    caxis([propMin propMax]);
    cbar.Limits = [0 1];
    cbar.Ticks=0:0.2:1;
