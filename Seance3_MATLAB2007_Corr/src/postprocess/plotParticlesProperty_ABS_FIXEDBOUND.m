...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%plotParticlesProperty_ABS_FIXEDBOUND(part,PROP,propMin,propMax,time,nameBar,scale)
% Boosted with patch
function plotParticlesProperty_ABS_FIXEDBOUND(part,PROP,propMin,propMax,time,nameBar,scale)
global dr lspace;
global INFO;
global POS;
global SURFACE FLUID BOUND;

infoTab = part(:,INFO);
fluid = part(infoTab ==FLUID,:);
bound = part(infoTab ==BOUND,:);
propTab= sum(fluid(:,PROP).*fluid(:,PROP),2).^.5/scale;
%CROP
